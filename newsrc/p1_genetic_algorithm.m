function p1 = p1_genetic_algorithm(population_size, mutation_rate, num_generations)
    ...
    for i = 1:population_size
        ...
        population(i).genome = genome_space(i);
    end
    
    % mutate the population. Add/subtract a randomly selected number
    % between 0 and 1 from uniform continuous distribution.
    for j = 1:num_generations
        for k = 1:population_size
            modified_genome = population(k).genome + random('unif',0,mutation_rate,1,1);
    ...
    [~, sorted_idx] = sort([population.fitness]);
    population = population(sorted_idx);
    p1 = population(sorted_idx(population_size));
end
