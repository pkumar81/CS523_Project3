...
function  longevity=longevity_no_image(p1, p2, Num, max_steps)
	   if (p1>0)&&(Num==0)&&(p2==0)
	      longevity=tree1(p1, max_steps);
       elseif (p1>0)&&(Num==0)&&(p2>0)
    ...
    cells=cells+lightning;  
      ...
      if longevity>0
         break;
          ...
          longevity=max_steps; %if the forest in not empty, set longevity to max_steps
      end
